...
def _format_override_value(value: Any) -> str:
    ...
    if isinstance(value, (int, float)):
        return str(value)
    # Lists / OmegaConf ListConfig / dicts → Python literal so the subprocess
    # can ast.literal_eval back to a real list (needed for step_skip_thresholds
    # and step_skip_sim_sources). repr(["dino", "pointmap"]) = "['dino', 'pointmap']".
    try:
        from omegaconf import ListConfig, DictConfig
        if isinstance(value, (ListConfig, DictConfig)):
            from omegaconf import OmegaConf
            return repr(OmegaConf.to_container(value, resolve=True))
    except ImportError:
        pass
...
def main(cfg: DictConfig):
    ...
    _append_override(overrides, "offload_text_encoder", cfg.EVALUATION.get("offload_text_encoder", False))
    _append_override(overrides, "torch_compile", cfg.EVALUATION.get("torch_compile", False))
    _append_override(overrides, "torch_compile_mode", cfg.EVALUATION.get("torch_compile_mode", "reduce-overhead"))
    _append_override(overrides, "torch_compile_scope", cfg.EVALUATION.get("torch_compile_scope", "step"))
    _append_override(overrides, "attn_backend", cfg.EVALUATION.get("attn_backend", "sdpa"))
    _append_override(overrides, "compile_encoders", cfg.EVALUATION.get("compile_encoders", False))
    _append_override(overrides, "torch_compile_backend", cfg.EVALUATION.get("torch_compile_backend", "inductor"))
    _append_override(overrides, "quantization", cfg.EVALUATION.get("quantization"))
    _append_override(overrides, "trt_joint_engine_path", cfg.EVALUATION.get("trt_joint_engine_path"))
    _append_override(overrides, "trt_joint_free_video_blocks", cfg.EVALUATION.get("trt_joint_free_video_blocks"))
    _append_override(overrides, "trt_joint_prefill_split_engine_path", cfg.EVALUATION.get("trt_joint_prefill_split_engine_path"))
    _append_override(overrides, "trt_joint_decode_split_engine_path", cfg.EVALUATION.get("trt_joint_decode_split_engine_path"))
    _append_override(overrides, "solver", cfg.EVALUATION.get("solver"))
    _append_override(overrides, "glue_cache", cfg.EVALUATION.get("glue_cache"))
    _append_override(overrides, "encoder_cuda_graph", cfg.EVALUATION.get("encoder_cuda_graph"))
    _append_override(overrides, "joint_loop_cuda_graph", cfg.EVALUATION.get("joint_loop_cuda_graph"))
    _append_override(overrides, "dynamic_step_skip", cfg.EVALUATION.get("dynamic_step_skip", False))
    _append_override(overrides, "step_skip_thresholds", cfg.EVALUATION.get("step_skip_thresholds"))
    _append_override(overrides, "step_skip_sim_sources", cfg.EVALUATION.get("step_skip_sim_sources"))
    _append_override(overrides, "step_skip_sim_aggregation", cfg.EVALUATION.get("step_skip_sim_aggregation", "mean"))
    _append_override(overrides, "infer_joint_video", cfg.EVALUATION.get("infer_joint_video"))
    _append_override(overrides, "infer_joint_dino", cfg.EVALUATION.get("infer_joint_dino"))
    _append_override(overrides, "infer_joint_pointmap", cfg.EVALUATION.get("infer_joint_pointmap"))
    _append_override(overrides, "infer_present_video", cfg.EVALUATION.get("infer_present_video"))
    _append_override(overrides, "infer_present_dino", cfg.EVALUATION.get("infer_present_dino"))
    _append_override(overrides, "infer_present_pointmap", cfg.EVALUATION.get("infer_present_pointmap"))
    # Forwarded like the flags above, so the launchers' pin actually reaches
    # deploy_policy. Absent -> None -> skipped, leaving the auto-detect from
