...
# Some datasets carry up to 3 independent language paraphrases per episode,
# stored as separate per-frame int-index columns (each indexes into `meta.tasks`,
# exactly like `task_index`). The standard LeRobot `task_index` column is a copy
# of paraphrase #1, so by default training only ever sees #1. When
# `sample_language_annotations` is on, the loader uniformly picks one available
# paraphrase per sample — matching how DreamZero (this data's source) trains on
# them. Datasets without these columns are unaffected (candidates stays empty).
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # When True, __getitem__ resolves the prompt by uniformly sampling one of
        # the available LANGUAGE_ANNOTATION_KEYS paraphrases instead of always
        # using task_index (= paraphrase #1). Set per-dataset via
        # MultiLeRobotDataset.set_sample_language_annotations. Default off.
        self.sample_language_annotations = False
    ...
    def _get_query_timestamps(
        ...
        for key in self.meta.video_keys:
            if query_indices is not None and key in query_indices:
                # hf_dataset[q] vs .select(q): ~100× faster on RoboTwin schema (no
                # DatasetInfo deepcopy). Same payload — verified bitwise.
    ...
    def _query_videos(self, query_timestamps: dict[str, list[float]], ep_idx: int) -> dict[str, torch.Tensor]:
        ...
        item = {}
        num_cameras = len(query_timestamps)

        if num_cameras <= 1:
            # Single camera: skip threading overhead
        ...
        # Multiple cameras: decode in parallel threads.
        # Both torchcodec and pyav release the GIL during C-level FFmpeg decode,
        # so threads achieve real parallelism for video I/O.
        def _decode_one(args):
            vid_key, query_ts = args
            video_path = self.root / self.meta.get_video_file_path(ep_idx, vid_key)
            frames = decode_video_frames(video_path, query_ts, self.tolerance_s, self.video_backend)
            return vid_key, frames.squeeze(0)

        with ThreadPoolExecutor(max_workers=num_cameras) as pool:
            for vid_key, frames in pool.map(_decode_one, query_timestamps.items()):
                item[vid_key] = frames
    ...
    def __getitem__(self, idx) -> dict:
        ...
        if self.sample_language_annotations:
            # Uniformly pick one of the episode's available paraphrases (some sets have
            # up to 3). Filter "not provided" (index 0; the few 2-annotation
            # episodes leave a slot there). python `random` is per-worker reseeded
            # by the DataLoader; np.random would correlate picks across workers.
            candidates = []
            for key in LANGUAGE_ANNOTATION_KEYS:
                if key in item:
                    ti = int(item[key].item())
                    if self.meta.tasks.get(ti, "not provided") != "not provided":
...
class MultiLeRobotDataset(torch.utils.data.Dataset):
    ...
    def set_sample_language_annotations(self, flag: bool):
        for dataset in self._datasets:
            dataset.sample_language_annotations = flag
